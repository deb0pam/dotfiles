...
do
   ...
   t=$(echo "Storage: $(df -h / | awk '/\// {print $4 "/" $2 " "}') RAM: $(free -h | awk '/Mem:/ {print $3 "/" $2}') CPU: $(top -bn1 | grep "Cpu(s)" | awk '{print $2 + $4"%"}')")
   xsetroot -name "$t vol:$vol bri:$brightness bat:$battery $b $ip $(date +"%A %d.%m.%y %H:%M:%S")"
   sleep 0.5
done
